...
class Day2 {
    ...
    public static String[] everything(String input){
        
        Pattern p = Pattern.compile("min: ([0-9]+[-]) max: ([-][0-9]+) letter: ([a-z][:]) password: ([:] [a-z]+)");
        Matcher m = p.matcher(input);
        
        String [] everything = new String [3];
        while (m.find()) {
            everything[0] = m.group("min");
            everything[1] = m.group("max");
            everything[2] = m.group("letter");
            everything[3] = m.group("password");
        }
    ...
    public static String[] finalTouches(String min, String max, String letter){
        
        Pattern pt = Pattern.compile("min: ([0-9]+) max: ([0-9+]) letter: ([a-z])");
        Matcher minm = pt.matcher(min);
        Matcher maxm = pt.matcher(max);
        Matcher letterm = pt.matcher(letter);
        
        String [] everything = new String [2];
        while (minm.find()) {
            String minOutput = minm.group(min);
            everything[0] = minOutput;
        }
        while (maxm.find()) {
            String maxOutput = maxm.group(max);
            everything[1] = maxOutput;
        }
        while (letterm.find()) {
            String letterOuput = letterm.group(letter);
            everything[2] = letterOuput;
        }
